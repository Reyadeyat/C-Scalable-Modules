...
#include <stdio.h>
#include "module_canvas_0_0_0.h"

void module_canvas_internal_process_0_0_0(Module_CANVAS_Data *module_canvas_data) {
    printf("Module A internal process version %s name %s buffer_size %u\n", module_canvas_data->version, module_canvas_data->name, module_canvas_data->buffer_size);
}

//external
void module_canvas_external_construct_v_0_0_0(Module_CANVAS_Data *module_canvas_data) {
    printf("Module A external construct version %s name %s buffer_size %u\n", module_canvas_data->version, module_canvas_data->name, module_canvas_data->buffer_size);
}

void module_canvas_external_destruct_v_0_0_0(Module_CANVAS_Data *module_canvas_data) {
    printf("Module A external destruct version %s name %s buffer_size %u\n", module_canvas_data->version, module_canvas_data->name, module_canvas_data->buffer_size);
}

int module_canvas_external_process_v_0_0_0(Module_CANVAS_Data *module_canvas_data) {
    printf("Module A external process version %s name %s buffer_size %u\n", module_canvas_data->version, module_canvas_data->name, module_canvas_data->buffer_size);
    module_canvas_internal_process_0_0_0(module_canvas_data);
    return 0;
}
